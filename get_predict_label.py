...
if __name__ == "__main__":
    ...
    run_config = tf.estimator.RunConfig(model_dir=hp.OUTPUT_DIR, save_summary_steps=hp.SAVE_SUMMARY_STEPS,
                                        save_checkpoints_steps=hp.SAVE_CHECKPOINTS_STEPS)
    bert_config = modeling.BertConfig.from_json_file(hp.BERT_CONFIG)
    model_fn = model_fn_builder(bert_config=bert_config, num_labels=len(label_list), learning_rate=hp.LEARNING_RATE,
    ...
    # tokenizer = create_tokenizer_from_hub_module(hp)
    # text = tokenizer.tokenize("This here's an example of using the BERT tokenizer")
    # print(text)
